...
from svd_solver import save_markdown_from_text, solve_text_to_markdown
...
class SVDApp(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("SVD Solver")
        self.geometry("1040x740")
        self.minsize(860, 580)

        self.input_path = Path("svd_input.txt")
        self.output_path = Path("svd_solution.md")
    ...
    def _build_widgets(self) -> None:
        ...
        ttk.Label(left, text="Nhập ma trận và tham số SVD").grid(row=0, column=0, sticky="w")
    ...
    def choose_output(self) -> None:
        path = filedialog.asksaveasfilename(
            ...
            initialfile=self.output_entry.get() or "svd_solution.md",
        )
        if path:
    ...
    def solve_and_export(self) -> None:
        output = Path(self.output_entry.get().strip() or "svd_solution.md")
        try:
...
def run_cli() -> int:
    parser = argparse.ArgumentParser(description="Giải SVD và xuất Markdown.")
    parser.add_argument("--input", "-i", help="File text chứa ma trận và tham số.")
    parser.add_argument("--output", "-o", default="svd_solution.md", help="File Markdown output.")
